...
void showRadioText() {
  ...
  if (!screenmute && radio.rds.hasRT && (radio.rds.stationText.length() > 0 || radio.rds.stationText32.length() > 0)) rtDisplay.update(RTString, RDSstatus, RDSColor, RDSColorSmooth, RDSDropoutColor, RDSDropoutColorSmooth, BackgroundColor);
  ...
  if (!screenmute) {
    ...
    if (radio.eon_counter > 0 && afpagenr > 1) {
      if (!haseonold) {
        ...
        tftPrint16(ACENTER, "TA", 250, 32, ActiveColor, ActiveColorSmooth);
        tftPrint16(ACENTER, "TP", 276, 32, ActiveColor, ActiveColorSmooth);
        tftPrint16(ACENTER, "PTY", 304, 32, ActiveColor, ActiveColorSmooth);
        haseonold = true;
      }
